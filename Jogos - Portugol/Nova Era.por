  ...
  funcao inicio()
  {
    ...
    enquanto (hp > 0 e cap <= 40) { 
      ...
      se (menu == 1) {
        ...
        inteiro hp_i_max = 550 * cap, hp_i = hp_i_max, atk_i = 60 + (cap * 30)
        dominio_ativo = falso
        ...
        enquanto (hp > 0 e hp_i > 0) {
          ...
          // --- IA DO INIMIGO ---
          inteiro acao_inimigo = u.sorteia(1, 100)
          inteiro dano_inimigo = 0
          
          se (nao dominio_inimigo_ativo e nao dominio_ativo e (cap >= 12)) {
             se (u.sorteia(1, 100) <= 20) { 
                escreva("\n⚠️ O INIMIGO REALIZA: RYŌIKI TENKAI! ⚠️\n")
                u.aguarde(1500)
          ...
          se (acao_inimigo > 0) {
              se (hp_i < (hp_i_max * 0.2) e acao_inimigo <= 40) {
                  escreva("🔥 O INIMIGO ESTÁ DESESPERADO! ATAQUE MASSIVO!\n")
                  dano_inimigo = (atk_i * 1.5) - (res/4)
              }
              senao {
                  dano_inimigo = atk_i - (res/4)
              }

              se (dominio_ativo) { dano_inimigo = dano_inimigo / 2 } 
              se (dominio_inimigo_ativo) { dano_inimigo = dano_inimigo * 2 escreva("💀 DANO DOBRADO PELO DOMÍNIO INIMIGO!\n") }
              
              se (dano_inimigo < 0) dano_inimigo = 0
              hp = hp - dano_inimigo
          }

          // --- EFEITOS E ANIMAÇÕES DAS EXPANSÕES ---
          se (dominio_ativo) { 
            se (dominio == "Vazio Imensuravel") {
                escreva("🤞 [", nome, "] 'Não toque em nada. Não pense em nada.'\n")
                escreva("♾️ EFEITO: Vazio Imensurável paralisa a existência!\n")
                d = d * 3 
                dano_inimigo = dano_inimigo / 2 
            }
            senao se (dominio == "Santuario Malevolente") {
                se (cla == "Itadori") {
                    escreva("🔪 [", nome, "] 'Corte... e Impacto!'\n")
                    escreva("⛩️ EFEITO: O Santuário do Vaso destrói tudo ao redor!\n")
                    d = (d * 2) + 100 
                    hp = hp + 50 se (hp > hp_max) hp = hp_max
                } senao {
                    escreva("🔪 [", nome, "] 'Fukuma Mizushi!'\n")
                    escreva("⛩️ EFEITO: Cortes automáticos Cleave & Dismantle!\n")
                    hp_i = hp_i - (atk * 2) 
                    d = d + (d / 2)
                }
            }
            senao se (dominio == "Templo do Crepusculo") {
                escreva("📜 [", nome, "] 'Que o crepúsculo apague sua alma.'\n")
                escreva("⛩️ EFEITO: Julgamento Ancestral de Sugawara!\n")
                d = d * 2.5
            }
            senao se (dominio == "Amor Autentico Mutuo") {
                escreva("💍 [", nome, "] 'Rika-chan... me empreste sua força.'\n")
                escreva("💍 EFEITO: O Amor Autêntico é imparável!\n")
                d = d * 2
                res = res + 100 
            }
            senao se (dominio == "Palacio das Sombras") {
                escreva("🌑 [", nome, "] 'Afunde na escuridão eterna.'\n")
                escreva("🌑 EFEITO: Você se torna uma sombra intocável!\n")
                d = d + 150
                dano_inimigo = dano_inimigo / 4
            }
            senao se (dominio == "Jardim das Sombras Quimericas") {
                escreva("🐺 [", nome, "] 'Emerjam das sombras!'\n")
                escreva("🐺 EFEITO: Shikigamis atacam de todos os ângulos!\n")
                d = d * 2.5
            }
            senao se (dominio == "Horizonte de Sangue") {
                escreva("🩸 [", nome, "] 'Meu sangue, sua morte.'\n")
                escreva("🩸 EFEITO: O Horizonte se torna carmesim!\n")
                hp = hp + (hp_max * 0.1)
                se (hp > hp_max) hp = hp_max
                d = d + (d / 2)
            }
            senao se (dominio == "Proporcao Divina") {
                escreva("📏 [", nome, "] 'Hora extra...'\n")
                escreva("📏 EFEITO: Proporção 7:3 aplicada em toda a área!\n")
                d = d * 2.2
            }
            senao se (dominio == "Carnificina Explosiva") {
                escreva("💣 [", nome, "] 'Boom.'\n")
                escreva("💣 EFEITO: Explosões constantes ignoram a resistência!\n")
                d = d + (atk_i)
            }
            senao { 
                escreva("✨ [", nome, "] EXPANSÃO DE DOMÍNIO ATIVA!\n")
                d = d + (d / 2) 
            }
          } 

          // --- ANIMAÇÃO BLACK FLASH ---
          se (d > 0 e u.sorteia(1, 100) <= 20) { 
            d = d * 2 
            escreva("\n⬛⚡⬛⚡⬛⚡⬛⚡⬛⚡⬛\n")
            escreva("⚡ KOKUSEN! BLACK FLASH! ⚡\n")
            escreva("⬛⚡⬛⚡⬛⚡⬛⚡⬛⚡⬛\n")
            u.aguarde(500)
          }
      ...
      senao se (menu == 6) { // STATUS DO DOMÍNIO
        ...
        senao se (dominio == "Santuario Malevolente") escreva("Efeito: Cortes automaticos e bonus de dano massivo (Especial para Itadori).\n")
        senao se (dominio == "Templo do Crepusculo") escreva("Efeito: Poder Ancestral - 2.5x de Dano total.\n")
        senao se (dominio == "Amor Autentico Mutuo") escreva("Efeito: Dano 2x e Resistência Máxima.\n")
        senao se (dominio == "Palacio das Sombras") escreva("Efeito: Ocultacao - Bonus de Dano e reduz muito o dano recebido.\n")
        ...
        senao se (dominio == "Proporcao Divina") escreva("Efeito: Multiplicador de dano critico 2.2x.\n")

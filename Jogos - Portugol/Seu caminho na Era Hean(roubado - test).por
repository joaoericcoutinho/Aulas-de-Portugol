programa
{
	...
	funcao inicio()
	{
		...
		enquanto (tent > 0) {
			...
			se (sorteio_raridade <= 100) { 
				raridade = "LENDÁRIO 🟡"
				inteiro r = u.sorteia(1, 2)
		...
		enquanto (tent > 0) {
			...
			se (cla == "Gojo") { se (sorteio_tec <= 100) { tecnica = "Ilimitado + Seis Olhos" } senao { tecnica = "Lapsus de Azul" } }
		...
		// --- STATUS BASE ---
		se (cla == "Gojo") { hp_max=650 en_max=1000 atk=100 con=200 res=80 } 
		...
		senao se (cla == "Okkotsu") { passiva_nome = "Copia Imparável" passiva_desc = "+100 Energia Max." en_max += 2000 }
		...
		enquanto (hp > 0 e cap <= 10) {
			...
			// Atribuição de Rank baseado no Level ou Capítulo
			se (lvl >= 8 ou cap >= 8) { rank = "Grau Especial 🔴" }
			...
			se (menu == 1) {
				limpa()
				// --- ADIÇÃO DE HISTÓRIA NARRATIVA ---
				se (cap == 1) { escreva("📜 O DESPERTAR: Você acorda em uma Kyoto devastada. Maldições menores vagam pelas ruas.\n") }
				senao se (cap == 3) { escreva("📜 A ASCENSÃO: O mundo jujutsu nota sua força. Maldições de Nível 1 foram enviadas para te parar.\n") }
				senao se (cap == 5) { escreva("📜 CAMINHO DE SANGUE: Você invade o templo central. Os anciões temem seu poder crescente.\n") }
				senao se (cap == 8) { escreva("📜 O TOPO DO MUNDO: Apenas as Maldições de Grau Especial restam entre você e o trono.\n") }
				senao se (cap == 10) { escreva("📜 O CONFRONTO FINAL: O Rei das Maldições te aguarda no horizonte de ossos.\n") }
				...
				enquanto (hp > 0 e hp_i > 0) {
					...
					se (dominio_ativo) { escreva("✨ DOMÍNIO ATIVO: ", dominio, " ✨\n") }
					...
					se (ac == 1) { 
						d = atk 
						se (cla == "Kamo") { hp += 20 se (hp > hp_max) hp = hp_max } // Efeito passiva Kamo
					}
					senao se (ac == 2 e (en >= 90 ou en_max == 0)) { 
						inteiro custo = 90
						se (cla == "Gojo") { custo = 45 } // Efeito passiva Gojo
						en = en - custo 
						d = (atk + con) * 2 
					}
					senao se (ac == 3 e en >= 250 e nao dominio_ativo e cla != "Fushiguro (Restricao)") {
						...
						se (cap >= 2) {
							...
							u.aguarde(1000)
							se (u.sorteia(1, 2) == 1) { dominio_ativo = verdadeiro escreva("VENCEU O CLASH!\n") } 
							senao { hp = hp - (hp_max*0.2) escreva("PERDEU O CLASH!\n") }
				...
				se (hp > 0) { 
					...
					se (cla == "Sugawara (Ancestral)") { exp_ganha = exp_ganha * 1.5 } // Efeito passiva Sugawara
					exp = exp + exp_ganha
			...
			senao se (menu == 3) { // VOTO VINCULATIVO
				...
				se (nao v6) escreva("6. Pacto de Heian: -200 Ienes por +70 ATK e +70 RES\n") senao escreva("6. [FEITO]\n")
				...
				senao se (ev == 6 e nao v6 e ienes >= 200) { ienes -= 200 atk += 70 res += 70 v6 = verdadeiro escreva("Voto selado!") }
			...
			senao se (menu == 4) { // BESTIARIO
				limpa()
				escreva("--- 📖 BESTIÁRIO ---\n")
				escreva("Cap 1-2: Maldições de Nível Baixo\n")
				escreva("Cap 3-4: Maldições de Nível 1\n")
				escreva("Cap 5+: Graus Especiais\n")
			...
			senao se (menu == 6) { // STATUS DO DOMÍNIO
				limpa()
				escreva("--- DOMÍNIO: ", dominio, " ---\n")
				escreva("Efeito: +50% de Dano e -50% de Dano Recebido.\n")
				leia(tecla)
			}
